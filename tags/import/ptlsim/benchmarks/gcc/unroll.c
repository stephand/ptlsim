     ...
     int strength_reduce_p;
{
  ...
  if (max_labelno > 0)
    {
      ...
      local_label = (char *) alloca ((size_t) max_labelno);
  ...
  local_regno = (char *) alloca ((size_t) max_reg_before_loop);
  ...
  if (unroll_type == UNROLL_NAIVE && ! splitting_not_safe && strength_reduce_p)
			       ...
			       loop_start, loop_end))
	{
	  ...
	  map->const_age_map = (unsigned *) alloca (maxregnum*sizeof (unsigned));
