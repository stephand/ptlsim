...
#ifdef _WIN64
__int64 reg_rtx_no = LAST_VIRTUAL_REGISTER + 1;
#else
long reg_rtx_no = LAST_VIRTUAL_REGISTER + 1;
#endif
...
#ifdef _WIN64
__int64 regno_pointer_flag_length;
#else
long regno_pointer_flag_length;
#endif
...
gen_rtx VPROTO((enum rtx_code code, enum machine_mode mode, ...))
{
  ...
  if (code == CONST_INT)
    ...
    {
#ifdef _WIN64
      __int64 regno = va_arg (p, int);
#else
      long regno = va_arg (p, int);
#endif
...
#else
	  CONST_DOUBLE_LOW(tem) = u.i[0];
	  CONST_DOUBLE_HIGH(tem) = u.i[1];
#endif
