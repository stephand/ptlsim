...
#define OBSTACK_CHUNK_FREE free
#endif
  _obstack_begin (obstack, (size_t) OBSTACK_CHUNK_SIZE, 0,
...
perm_calloc (nelem, size)
     ...
     size_t size;
{
  char *rval = (char *) obstack_alloc (&permanent_obstack,(size_t) nelem * size);
  bzero (rval,(size_t) nelem * size);
  return rval;
}
     ...
     tree type;
{
  ...
  h = (struct type_hash *) oballoc ((size_t) sizeof (struct type_hash));
  h->hashcode = hashcode;
  h->type = type;
     ...
     int wd_size;
{
  ...
  char *bit_buffer = (char*)alloca((size_t)bit_size);
