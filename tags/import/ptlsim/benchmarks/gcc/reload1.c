...
static int hard_reg_use_compare		PROTO((const void *, const void *));
static void order_regs_for_reload	PROTO((void));
static int compare_spill_regs		PROTO((const void *, const void *));
static void reload_as_needed		PROTO((rtx, int));
static void forget_old_reloads_1	PROTO((rtx, rtx));
static int reload_reg_class_lower	PROTO((const void *, const void *));
     ...
     FILE *dumpfile;
{
  ...
  cannot_omit_stores = (char *) alloca ((size_t)max_regno);
  bzero (cannot_omit_stores,(size_t) max_regno);
  ...
  offsets_known_at = (char *) alloca ((size_t)num_labels);
  offsets_at
  ...
  if (global)
    for (i = 0; i < N_REG_CLASSES; i++)
      {
	basic_block_needs[i] = (char *) alloca ((size_t)n_basic_blocks);
...

static int
hard_reg_use_compare (cvp1, cvp2)
     const void *cvp1, *cvp2;
{
  struct hard_reg_n_uses *p1 = (struct hard_reg_n_uses *) cvp1;
  struct hard_reg_n_uses *p2 = (struct hard_reg_n_uses *) cvp2;
     ...
     int live_known;
{
  ...
  reg_has_output_reload = (char *) alloca ((size_t)max_regno);
  for (i = 0; i < n_spills; i++)
    {
...
static int
reload_reg_class_lower (cvp1, cvp2)
     const void *cvp1, *cvp2;
{
  register int r1 = *((short *)cvp1), r2 = *((short *)cvp2);
  register int t;
