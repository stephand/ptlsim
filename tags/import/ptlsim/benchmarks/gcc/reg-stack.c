     ...
     FILE *file;
{
  ...
  block_begin = (rtx *) alloca ((size_t)blocks * sizeof (rtx));
  block_end = (rtx *) alloca ((size_t)blocks * sizeof (rtx));
  block_drops_in = (char *) alloca ((size_t)blocks);

  block_stack_in = (stack) alloca ((size_t)blocks * sizeof (struct stack_def));
  block_out_reg_set = (HARD_REG_SET *) alloca ((size_t)blocks * sizeof (HARD_REG_SET));
  bzero ((char *) block_stack_in, blocks * sizeof (struct stack_def));
  bzero ((char *) block_out_reg_set, blocks * sizeof (HARD_REG_SET));

  block_number = (int *) alloca ((size_t)(max_uid + 1) * sizeof (int));
...
{
  char **constraints = (char **) alloca ((size_t)n_operands * sizeof (char *));
  char *q;
     ...
     int n_inputs, n_outputs;
{
  ...
  int *operand_matches = (int *) alloca ((size_t)n_operands * sizeof (int *));

  enum reg_class *operand_class 
    = (enum reg_class *) alloca ((size_t)n_operands * sizeof (enum reg_class *));
  ...
  if (GET_CODE (body) == PARALLEL)
    {
      clobber_reg = (rtx *) alloca ((size_t)XVECLEN (body, 0) * sizeof (rtx *));
     ...
     int n_inputs, n_outputs;
{
  ...
  note_reg = (rtx *) alloca ((size_t)i * sizeof (rtx));
  note_loc = (rtx **) alloca ((size_t)i * sizeof (rtx *));
  note_kind = (enum reg_note *) alloca ((size_t)i * sizeof (enum reg_note));
  ...
  if (GET_CODE (body) == PARALLEL)
    {
      clobber_reg = (rtx *) alloca ((size_t)XVECLEN (body, 0) * sizeof (rtx *));
      clobber_loc = (rtx **) alloca ((size_t)XVECLEN (body, 0) * sizeof (rtx **));
     ...
     stack regstack;
{
  ...
  if (GET_MODE (insn) == QImode)
      ...
      if (n_operands >= 0)
	{
	  ...
	  int n_inputs, n_outputs;
	  char **constraints
	    = (char **) alloca ((size_t)n_operands * sizeof (char *));
