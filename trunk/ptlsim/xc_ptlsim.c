...
int setup_ptlsim_space(int xc_handle, uint32_t dom, mfn_t* mfns, int count,
  ...
  struct LongModeLevel1PTE* ptl_level1;
  pfn_t ptl_level1_pfn;

  struct LongModeLevel2PTE* ptl_level2;
  mfn_t ptl_level2_mfn;

  struct LongModeLevel3PTE* ptl_level3;
  mfn_t ptl_level3_mfn;

  struct LongModeLevel4PTE* level4;
  mfn_t level4_mfn;

  W64 total_machine_pages;

  struct LongModeLevel1PTE* physmap_level1;
  W64 physmap_level1_pages;
  pfn_t physmap_level1_pfn;

  struct LongModeLevel2PTE* physmap_level2;
  W64 physmap_level2_pages;
  pfn_t physmap_level2_pfn;

  struct LongModeLevel3PTE* physmap_level3;
...
#define virt_to_pfn(virt) ((((byte*)(virt)) - reserved_page_pool) / PAGE_SIZE)
  ...
  alloc_pages(LongModeLevel1PTE, ptl_level1, pagedir_pages_required);

  ptl_level1_pfn = virt_to_pfn(ptl_level1);

  bootpage->ptl_pagedir = (struct LongModeLevel1PTE*)(PTLSIM_VIRT_BASE + ((byte*)ptl_level1 - reserved_page_pool));

  for (i = 0; i < count; i++) {
    struct LongModeLevel1PTE* pte = &ptl_level1[i];
    pte->p = 1;
  ...
  alloc_page(LongModeLevel2PTE, ptl_level2, ptl_level2_mfn);

  for (i = 0; i < pagedir_pages_required; i++) {
    struct LongModeLevel2PTE* pte = &ptl_level2[i];
    pte->p = 1;
  ...
  bootpage->ptl_pagedir_map = (struct LongModeLevel2PTE*)(PTLSIM_VIRT_BASE + ((byte*)ptl_level2 - reserved_page_pool));
  bootpage->ptl_pagedir_map_mfn = ptl_level2_mfn;

  alloc_page(LongModeLevel3PTE, ptl_level3, ptl_level3_mfn);
  ...
  alloc_pages(LongModeLevel1PTE, physmap_level1, physmap_level1_pages);
  physmap_level1_pfn = virt_to_pfn(physmap_level1);

  bootpage->phys_pagedir = (struct LongModeLevel1PTE*)(PTLSIM_VIRT_BASE + ((byte*)physmap_level1 - reserved_page_pool));

  physmap_level2_pages = (physmap_level1_pages + (PTES_PER_PAGE-1)) / PTES_PER_PAGE;

  alloc_pages(LongModeLevel2PTE, physmap_level2, physmap_level2_pages);
  physmap_level2_pfn = virt_to_pfn(physmap_level2);

  for (i = 0; i < physmap_level1_pages; i++) {
    struct LongModeLevel2PTE* pte = &physmap_level2[i];
    pte->p = 1;
    pte->rw = 1; // sub-pages are writable unless overridden
    pte->us = 1; // both user and supervisor
    pte->a = 1;
  ...
  alloc_page(LongModeLevel3PTE, physmap_level3, physmap_level3_mfn);

  for (i = 0; i < physmap_level2_pages; i++) {
    struct LongModeLevel3PTE* pte = &physmap_level3[i];
    pte->p = 1;
    pte->rw = 1; // sub-pages are writable unless overridden
    pte->us = 1; // both user and supervisor
    pte->a = 1;
...
#define INCLUDE_PHYS_MEM_MAPPINGS
  ...
  alloc_page(LongModeLevel4PTE, level4, level4_mfn);

  {
    int index = PTLSIM_VIRT_BASE >> (12+9+9+9);
    struct LongModeLevel4PTE* top = &level4[index];
...
#ifdef INCLUDE_PHYS_MEM_MAPPINGS
  {
  ...
  bootpage->toplevel_page_table = (struct LongModeLevel4PTE*)(PTLSIM_VIRT_BASE + ((byte*)level4 - reserved_page_pool));
