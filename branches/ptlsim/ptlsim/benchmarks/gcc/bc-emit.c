     ...
     int log;
{
  ...
  if (seg->size > seg->alloc)
	...
	seg->alloc *= 2;
      seg->data = xrealloc (seg->data, (size_t) seg->alloc);
    }
...
/* It's also required only on GNU compilers older than
   about 2.6, and breaks newer ones like 2.95 on some architectures, 
   so SPEC altered the ifdef.   MJP for SPEC	*/
#ifdef __OLDANDBUGGY__GNUC__
