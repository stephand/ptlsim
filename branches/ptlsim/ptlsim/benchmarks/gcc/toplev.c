...
int flag_writable_strings = 1;	/* MJP */
...
output_lang_identify (asm_out_file)
     FILE *asm_out_file;
{
  size_t len = strlen (lang_identify ()) + sizeof ("__gnu_compiled_") + 1;
  char *s = (char *) alloca ((size_t)len);
     ...
     tree decl;
{
  ...
  if (DECL_SAVED_INSNS (decl) == 0)
	...
	TIMEVAR (integration_time,
		 {
		   ...
		   if (lose || ! optimize)
		     {
		       if (warn_inline && (/*MJP*/specd||flag_inline_functions))
      ...
      if (DECL_INLINE (decl) || DECL_DEFER_OUTPUT (decl))
	{
      ...
      if ((/*MJP*/specd||DECL_INLINE(decl)) && DECL_EXTERNAL (decl))
	goto exit_rest_of_compilation;
    }
     ...
     char **argv;
{
  ...
  flag_omit_frame_pointer = 1;		/* MJP make this a default */
  ...
  for (i = 1; i < argc; i++)
    ...
    optimize = 3;	/* MJP Ensure -O3 for Spec2000 */
...
/*    flag_caller_saves = 1;		MJP	Not for Spec2000 */
      flag_force_mem = 1;
  ...
  for (i = 1; i < argc; i++)
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0)
	{
	  ...
	  if (str[0] == 'm')
	    ...
	    {
#if 0
/* do not allow overrides of these for Spec2000 */
	      register char *p = &str[1];
	      int found = 0;
