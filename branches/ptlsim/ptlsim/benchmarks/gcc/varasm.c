...
#ifdef _WIN64
  obstack_1grow (&permanent_obstack, (unsigned __int64) code);
#else
  obstack_1grow (&permanent_obstack, (unsigned long) code);
#endif
...
#define SYMHASH(LABEL)	\
  ((((HOST_WIDE_INT) (LABEL)) & (((HOST_WIDE_INT)1 << HASHBITS) - 1))  % MAX_RTX_HASH_TABLE)

     ...
     register int size;
{
  ...
  switch (code)
	...
	{
	  unsigned char *buffer = (unsigned char *) alloca ((size_t)size);
